...
import type { HowItWorksConfig, WorkflowStepConfig } from '../../../types';

export const HowItWorksEditor: React.FC<{ onPreview: () => void }> = ({ onPreview }) => {
  const { content, saveSection, isSaving } = useContent();
  const [data, setData] = useState<HowItWorksConfig>(() =>
    JSON.parse(JSON.stringify(content.howItWorks))
  );
  const [saveStatus, setSaveStatus] = useState<'idle' | 'success' | 'error'>('idle');

  const hasChanges = JSON.stringify(data) !== JSON.stringify(content.howItWorks);

  const handleSave = async () => {
    try {
      setSaveStatus('idle');
      const ok = await saveSection('howItWorks', data);
      if (ok) {
  ...
  const handleAddStep = () => {
    const nextNum = String(data.steps.length + 1).padStart(2, '0');
    const newStep: WorkflowStepConfig = {
      step: nextNum,
      title: 'New Workflow Step',
      description: 'Describe this phase of working with ELEVATE.',
      icon: 'Send',
      enabled: true,
    };
  ...
  return (
    <div className="space-y-6">
      <AdminSectionHeader
        ...
        description="Configure the sequential 5-step operational workflow displayed to prospective institutional partners."
        isSaving={isSaving}
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="text-base font-bold font-['Outfit'] text-white">
              Workflow Steps ({data.steps.length})
            </h3>
            <p className="text-xs text-slate-400">Step number, title, and description</p>
          </div>
          <button
            type="button"
